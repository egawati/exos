    ...
    while True:
        ...
        try:
            ...
            if hash_d is None:
                ...
                print(f'at estimator, finally get all signal from OA and value is {value.value}')
                value.set_value(n_streams)
                print(f"Ready to waking up temporal neighbor, resetting value to {value.value}\n")
                with neigh_condition:
                    neigh_condition.notify_all()
                print("estimator --> temporal neighbor woken\n")
        except Exception as e:
            print(f'Estimator exception is {e}\n')
    value.set_value(-1)
    print(f'estimator {pid} exit, value is {value.value}\n')
    #sys.stdout.flush()
