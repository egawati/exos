...
def concatenate_buffers(hash_d, n_streams, arrival_rates):
    """
    ...
    new_hash_d = {}
    stream_ids = hash_d.keys().sort()
    for stream_id in hash_d.keys():
        a = arrival_rates[stream_id]
        arr = hash_d[stream_id]
        filtered_indices = np.where(arr[:, 0] % a == 0)[0]
        new_hash_d[stream_id] = arr[filtered_indices]
    concat_arr = np.hstack()
    return concat_arr
...
def run_dbpca_estimator(value, neigh_condition, exos_condition, 
    ...
    while True:
        ...
        try:
            ...
            if hash_d is None:
                ...
                arr = concatenate_buffers(hash_d, n_streams, arrival_rates)
