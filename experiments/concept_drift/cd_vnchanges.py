...
def run_experiments(case, 
                    window_size = 1000,
                    n_streams=4,
                    relpath='Results/exos', 
                    data_folder='../Datasets/ConceptDrift/WindowChanges/NChanges'):
    cwd = os.getcwd()
    round_flag = False
    multiplier = 5
    threshold= 0.0
...
if __name__ == '__main__':
    ...
    for case in cases:
        run_experiments(case=case, 
                        window_size = 1000,
                        n_streams=4,
                        relpath=f'Results/exos/{folder}', 
                        data_folder=f'../Datasets/ConceptDrift/WindowChanges/{folder}')
