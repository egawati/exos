...
from .unsync_generator import stream_producer, stream_consumer
from .unsync_estimator import run_dbpca_unsync_estimator
...
def run_unsync_exos_simulator(sources, d, k, attributes, feature_names, 
                              window_size, arrival_rates, n_clusters = (), n_init_centroids = (), 
    ...
    producers = [Process(target=stream_producer, 
                         args=(condition, queues[i], sources[i], i, window_size, arrival_rates[i]), 
    ...
    estimator_p = Process(target=run_dbpca_unsync_estimator,
